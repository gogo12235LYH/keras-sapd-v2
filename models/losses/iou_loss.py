...
def iou_mask_v2(mode='fciou', factor=1.0):
    ...
    def iou_mask_(inputs):
        ...
        return factor * tf.reduce_sum(mask * iou_loss) / normalizer
...
class IoULoss(keras.layers.Layer):
    def __init__(self, mode='fciou', factor=1.0, name='loc_loss', test=False, **kwargs):
        super(IoULoss, self).__init__(dtype='float32', name=name, **kwargs)
        self.mode = mode
        self.factor = factor
        ...
        if not test:
            ...
            self.fnc = iou_mask_v3(mode, factor)
